...
export async function generateMetadata(
  ...
  const product = await fetch(`https://genshin-db-api.vercel.app/api/v5/artifacts?query=${id.replace(/-/g, '')}&resultLanguage=portuguese`, { next: { revalidate: 60 } }).then((res) => res.json())
...
export default async function Page({params}:any) {
    ...
    const validIds = await fetch('https://genshin-db-api.vercel.app/api/v5/artifacts?query=names&matchCategories=true', { next: { revalidate: 60 } })
      .then(res => res.json());
