...
export async function generateMetadata(
  ...
  const product = await fetch(`https://genshin-db-api.vercel.app/api/v5/artifacts?query=${id.replace(/-/g, '')}&resultLanguage=portuguese`, { cache: 'force-cache' }).then((res) => res.json())
...
export default async function Page({params}:any) {
    ...
    const validIds = await fetch('https://genshin-db-api.vercel.app/api/v5/artifacts?query=names&matchCategories=true', { cache: 'force-cache' })
      .then(res => res.json());
    ...
    const [ptData, enData] = await Promise.all(
      urls.map(endpoint =>
        fetch(`https://genshin-db-api.vercel.app/api/v5/${endpoint}`, { cache: 'force-cache' }).then(res => res.json())
      )
    );
